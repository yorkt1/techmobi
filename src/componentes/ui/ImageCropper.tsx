...
async function createImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    // força re-fetch com CORS limpo quando é URL externa
...
export default function ImageCropper({ imageSrc, onConfirm, onCancel }: ImageCropperProps) {
  ...
  const [aspectKey, setAspectKey] = useState(0); // 4:3 por padrão
  ...
  const ui = (
    ...
    >
      ...
      <div style={{ position: "relative", flex: 1 }}>
        {/* key força re-mount ao trocar proporção — garante reset correto */}
        <Cropper
          key={`crop-${aspectKey}`}
          image={imageSrc}
          crop={crop}
